...
class TestInvQuadNonBatch(unittest.TestCase):
    ...
    def test_inv_quad_vector(self):
        ...
        with linear_operator.settings.num_trace_samples(1000):
            dense_linear_op = DenseLinearOperator(self.mat, linear_solver=CGGpytorch(tol=1e-5))
...
class TestInvQuadBatch(unittest.TestCase):
    ...
    def test_inv_quad_many_vectors(self):
        ...
        with linear_operator.settings.num_trace_samples(2000):
            dense_linear_op = DenseLinearOperator(self.mats, linear_solver=CGGpytorch(tol=1e-5))
            res_inv_quad = linear_operator.inv_quad(dense_linear_op, self.vecs)
